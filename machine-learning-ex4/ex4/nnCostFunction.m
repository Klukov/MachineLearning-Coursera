...
d3 = a3 - y;                                                          %matrix (m, out)
d2 = d3*Theta2(:,2:(size(Theta2,2))) .* sigmoidGradient(z2);          %matrix (m, hidden)

delta1 = zeros(size(Theta1));
delta2 = zeros(size(Theta2));

for i = 1:m
    delta2 = delta2 + a2(i,:) .* ((d3(i,:))');
    delta1 = delta1 + a1(i,:) .* ((d2(i,:))');
end
